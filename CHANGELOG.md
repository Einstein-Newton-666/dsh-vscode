...
- **修复代理转发一律 403（Origin 校验）**：面板 iframe 从代办端口加载，浏览器对同源 POST 必带 `Origin=<代办端口>`，而上游 CSRF 校验要求 `Origin` 等于自身 origin——页面本身正常、所有 API 调用却一律 403（GET 不带 Origin，故无明显征兆）。代理现把 `Origin` 重写为上游 origin，HTTP 转发与 WebSocket 升级两条路径同步处理，并补回归测试。
